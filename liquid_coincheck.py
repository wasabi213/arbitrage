...
class BrokerArbiTrage:
    ...
    def primaryBuySecondarySell(self, lot, p_ask, s_bid):
        ...
        LOG.critical("####################################################")
        LOG.critical(self.PRIMARY_BROKER + ":買い " +
                     self.SECONDARY_BROKER + ":売り")
        LOG.critical("####################################################")
    ...
    def primarySellSecondaryBuy(self, lot, s_ask, p_bid):
        ...
        LOG.critical("####################################################")
        LOG.critical(self.PRIMARY_BROKER + ":売り " +
                     self.SECONDARY_BROKER + ":買い")
        LOG.critical("####################################################")
    ...
    def forwardTrade(self, p_info, s_info):
        ...
        if self.primary_tradable_time >= self.price_over_count:
            ...
            self.primaryBuySecondarySell(
                self.btc_lot, p_info['tradable_ask'], s_info['tradable_bid'])
            self.primary_tradable_time = 0
            self.secondary_tradable_time = 0

        elif self.secondary_tradable_time >= self.price_over_count:

            self.primarySellSecondaryBuy(
                self.btc_lot, p_info['tradable_bid'], s_info['tradable_ask'])
            self.primary_tradable_time = 0
            self.secondary_tradable_time = 0
    ...
    def showConsoleLog(self, balance, p_info, s_info, pt_info, st_info):
        ...
        LOG.critical(self.SECONDARY_BROKER + ":" +
                     str(balance['secondary_jpy']))
        LOG.critical(self.PRIMARY_BROKER + "-BTC:" +
                     str(balance['primary_btc']))
        LOG.critical(self.SECONDARY_BROKER + "-BTC:" +
                     str(balance['secondary_btc']))
        LOG.critical("TOTAL JPY:" +
                     str(balance['primary_jpy'] + balance['secondary_jpy']))
        LOG.critical("TOTAL BTC:" +
                     str(balance['primary_btc'] + balance['secondary_btc']))
        LOG.critical(
            "-----------------------------------------------------------------")
        ...
        LOG.critical(self.PRIMARY_BROKER + ":bid-" + self.SECONDARY_BROKER +
                     ":ask " + str(float(p_info['bids'][0][0]) - float(s_info['asks'][0][0])))
        LOG.critical(self.SECONDARY_BROKER + ":bid-" + self.PRIMARY_BROKER +
                     ":ask " + str(float(s_info['bids'][0][0]) - float(p_info['asks'][0][0])))
        LOG.critical("-------------------- Tradable Price Difference")
        LOG.critical(self.PRIMARY_BROKER + ":bid-" + self.SECONDARY_BROKER + ":ask " +
                     str(float(pt_info['tradable_bid']) - float(st_info['tradable_ask'])))
        LOG.critical(self.SECONDARY_BROKER + ":bid-" + self.PRIMARY_BROKER + ":ask " +
                     str(float(st_info['tradable_bid']) - float(pt_info['tradable_ask'])))
    ...
    def mainProcess(self):
        ...
        while True:
            ...
            # トレード可能なロット数と価格を取得する。
            primary_tradable_info = self.getTradableInfo(
                primary_info, self.board_count)
            secondary_tradable_info = self.getTradableInfo(
                secondary_info, self.board_count)
            ...
            if self.action_mode == 'forward':
                ...
                self.countForwardTradableTime(
                    primary_tradable_info, secondary_tradable_info)

                # 通常方向へのトレードを行う。
                self.forwardTrade(primary_tradable_info,
                                  secondary_tradable_info)
            ...
            # JPY残高または、BTC残高が足りない場合は、リバースモード（偏り解消モード）に入る。
                ...
                # トレード可能な状態になっている回数をカウントしてプロパティに設定する。
                self.countReverseTradableTime(
                    primary_tradable_info, secondary_tradable_info)

                # リバース方向へのトレードを行う。
                self.reverseTrade(
                    trade_status, primary_tradable_info, secondary_tradable_info)

            self.showConsoleLog(self.balance, primary_info, secondary_info,
                                primary_tradable_info, secondary_tradable_info)
