...
class StatusMonitor:
    ...
    def __init__(self,primary,secondary):
        ...
        self.monitor_file_name  = self.primary_broker + '_' + self.secondary_broker + '_monitor.log'
    ...
    def setInfoforZaifCoincheck(self,info):
        ...
        total_jpy = "{:.0f}".format(float(info["balance"]["zaif_jpy"] ) + float(info["balance"]["coin_jpy"]))
        total_btc = "{:.3f}".format(float(info["balance"]["zaif_btc"] ) + float(info["balance"]["coin_btc"]))
        btc_price  = float(info["board"]["zaif_bid"][0]) if float(info["board"]["zaif_bid"][0]) < float(info["board"]["coin_bid"][0]) else float(info["board"]["coin_bid"][0])
        ...
        data = {
                ...
                "primary_safety_ask": info['board']['coin_tradable_ask_price'],
                "primary_safety_ask_lot": info['board']['coin_tradable_ask_lot_total'],
                "primary_safety_bid": info['board']['coin_tradable_bid_price'],
                "primary_safety_bid_lot": info['board']['coin_tradable_bid_lot_total'],
